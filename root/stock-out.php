<?php
// root/stock-out.php — Manual + Sheet tab
$page = 'stock-out';
?>
<!DOCTYPE html>
<html lang="en">
...
<title>Stock Out — Stock System</title>
...
<div class="app">
  ...
  <main>
    <div class="pagehead">
      <div>
        <h1>Stock Out</h1>
        <p class="desc">Manual entry ya sheet upload se stock nikaalo</p>
      </div>
    </div>
  ...
  fetch('../ajax/search_product.php?serial='+encodeURIComponent(s))
    ...
    .then(function(d){
      if (d.success && d.data){
        ...
        $('m_stock').value = d.data.current_stock;
        $('m_lookup').textContent = 'Mila. Available stock: ' + d.data.current_stock;
      } else {
        $('m_name').value = ''; $('m_stock').value = '';
        $('m_lookup').textContent = 'Product register nahi hai — pehle Stock In ya register karo.';
      }
    });
});
...
$('btnSubmitOut').addEventListener('click', function(){
  ...
  postForm('../ajax/stock_out_form.php', fd, function(d){
    if (d.success){
      $('m_qty').value = 1; $('m_remark').value = '';
      $('m_photo').value = ''; $('m_serial').value = '';
      $('m_name').value = ''; $('m_stock').value = '';
      $('m_lookup').textContent = 'Serial daalo to product info auto-aa jayegi.';
      $('m_serial').focus();
    }
  });
});
